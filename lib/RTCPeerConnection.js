...
RTCPeerConnection.prototype.createOffer = function(successCallback, failureCallback, options) {
	...
	var self = this;
	var disableTrickleIce = false;

	if (options && options.disableTrickleIce) {
		disableTrickleIce = !!(options.disableTrickleIce);
		// remove custom option
		delete options.disableTrickleIce;
	}
	...
	function createOfferSucceeded(offer) {
		if (!disableTrickleIce || self.iceGatheringState === 'complete') {
			if (successCallback) { successCallback(offer); }
			return;
		}
		var onicecandidate = function(event) {
			if (!event.candidate) {
				self.pc.removeEventListener('icecandidate', onicecandidate);
				if (successCallback) { successCallback(offer); }
			}
		};
		self.pc.addEventListener('icecandidate', onicecandidate);
    }
};
...
RTCPeerConnection.prototype.createAnswer = function(successCallback, failureCallback, options) {
	...
	function createAnswerSucceeded(answer) {
		if (!disableTrickleIce || self.iceGatheringState === 'complete') {
